...
class FiltersetRepository:
    """Lightweight helper that manages filtersets.json entries."""

    def __init__(self, config_path: Path | None = None) -> None:
        self.path = (config_path or Path("filtersets.json")).resolve()
        self.path.parent.mkdir(parents=True, exist_ok=True)
        if not self.path.exists():
            self._write_entries([json.loads(json.dumps(_DEFAULT_FILTERSET))])

    # ------------------------------------------------------------------
    # High-level CRUD
    # ------------------------------------------------------------------
    def list_filtersets(self) -> list[FiltersetRecord]:
        return [self._entry_to_record(entry) for entry in self._read_entries()]

    def get_entry(self, name: str) -> FiltersetRecord:
        entry = self._find_entry(name)
        if entry is None:
            raise KeyError(f"Filterset '{name}' not found")
        return self._entry_to_record(entry)

    def create_filterset(self, name: str, description: str = "") -> FiltersetRecord:
        normalized = self._sanitize(name)
        if not normalized:
            raise ValueError("Filterset name must not be empty")
        entries = self._read_entries()
        if self._find_entry(normalized, entries) is not None:
            raise ValueError(f"Filterset '{normalized}' already exists")
        entry = {
            "name": normalized,
    ...
    def delete_filterset(self, name: str) -> None:
        ...
        if len(filtered) == len(entries):
            raise KeyError(f"Filterset '{name}' not found")
        if not filtered:
            filtered = [json.loads(json.dumps(_DEFAULT_FILTERSET))]
        self._write_entries(filtered)

    def import_file(self, source: Path) -> list[FiltersetRecord]:
        ...
        if not new_entries:
            raise ValueError("Import file does not describe any filtersets")
        existing = self._read_entries()
        imported: list[FiltersetRecord] = []
    ...
    # ------------------------------------------------------------------
    def list_blocks(self, filterset: str) -> list[dict[str, Any]]:
        entry = self._find_entry(filterset)
        if entry is None:
            raise KeyError(f"Filterset '{filterset}' not found")
        return list(entry.get("blocks", []))

    def add_block(self, filterset: str, block: dict[str, Any]) -> dict[str, Any]:
        entries = self._read_entries()
        entry = self._require_entry(entries, filterset)
        new_block = self._normalize_block(block)
        entry.setdefault("blocks", []).append(new_block)
        self._write_entries(entries)
        return new_block

    def update_block(self, filterset: str, block_id: str, params: dict[str, Any]) -> dict[str, Any]:
        entries = self._read_entries()
        entry = self._require_entry(entries, filterset)
    ...
    def delete_block(self, filterset: str, block_id: str) -> None:
        entries = self._read_entries()
        entry = self._require_entry(entries, filterset)
        blocks = entry.setdefault("blocks", [])
    ...
    def _read_entries(self) -> list[dict[str, Any]]:
        if not self.path.exists():
            return [json.loads(json.dumps(_DEFAULT_FILTERSET))]
    ...
    def _write_entries(self, entries: Iterable[dict[str, Any]]) -> None:
        ...
        payload = {"filtersets": safe_entries}
        self.path.write_text(json.dumps(payload, indent=2), encoding="utf-8")

    def _coerce_to_entries(self, data: Any) -> list[dict[str, Any]]:
        if isinstance(data, dict):
            # Support both new and old key names
            if isinstance(data.get("filtersets"), list):
                entries = data["filtersets"]
            elif isinstance(data.get("manufacturers"), list):
            ...
            raise ValueError("Unsupported filterset config format")
        normalized = [self._normalize_entry(entry) for entry in entries if entry.get("name")]
        return normalized or [json.loads(json.dumps(_DEFAULT_FILTERSET))]
